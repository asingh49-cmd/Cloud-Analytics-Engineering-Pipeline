...
SELECT
    ...
    df.film_key,
    dst.store_key,
    dd_rental.date_key AS rental_date_key,
    dd_return.date_key AS return_date_key,
    1 AS count_rentals,
    CASE WHEN r.return_date IS NOT NULL THEN 1 ELSE 0 END AS count_returns,
    f.rental_duration,
    p.amount AS dollar_amount
FROM sakila.rental r
    #join film with inventory table
    JOIN sakila.inventory i ON r.inventory_id = i.inventory_id
    JOIN sakila.film f ON i.film_id = f.film_id
    
    #join with dimension tables
    JOIN sakila_snowflake.dim_customer dc ON r.customer_id = dc.customer_id
    JOIN sakila_snowflake.dim_staff ds ON r.staff_id = ds.staff_id
    JOIN sakila_snowflake.dim_film df ON f.film_id = df.film_id
    JOIN sakila_snowflake.dim_store dst ON i.store_id = dst.store_id

    #join date dimensions for rental and return
    JOIN sakila_snowflake.dim_date dd_rental ON DATE(r.rental_date) = dd_rental.date
    LEFT JOIN sakila_snowflake.dim_date dd_return ON DATE(r.return_date) = dd_return.date

    #left join payment for dollar_amount with rental
